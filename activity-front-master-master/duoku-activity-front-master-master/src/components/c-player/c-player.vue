<template>
  <div class="video-player" :id="id">
    <div class="video-wrap">
      <video :id ="id +'_video'" :volume="volume" :mute="mute" :autoplay="autoplay"
             :preload="preload" :poster="poster"  name="media" :loop="loop" :width="width" :height="height"
             webkit-playsinline="true"
             x-webkit-airplay="true"
             x5-playsinline="true"
             playsinline
      >
        <source :src="url" type="video/mp4" x5-video-player-type>
      </video>
    </div>
...
export default {
  ...
  methods: {
    initOptions () {
      let $vp = document.getElementById(this.id)
      let $v = document.getElementById(this.id + '_video')
      var params = {
        ...
        width: this.width,
        height: this.height,
        'x5-video-player-type': this.x5VideoPlayerType
      }
      new VideoControl($vp, $v, params).init()
    }
  },
  mounted () {
    this.initOptions()
