...
;(function () {
    ...
    var postURL = 'http://state.duoku.com/bilog'
    function post(data) {
        var xmlhttp = null;
        ...
        if (xmlhttp) {
            ...
            xmlhttp.send("action=booking&data="+data);
        }
        else {
    ...
    function sendLog(params, callback) {
        ...
        var base64str = base64.encode(JSON.stringify(params));
        post(base64str)
    }
    ...
    if(_dklog.action){
        postURL = postURL.replace(/{action}/,_dklog.action)
    }
